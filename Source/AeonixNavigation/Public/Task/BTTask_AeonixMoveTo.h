...
struct FBTAeonixMoveToTaskMemory
{
	/** Move request ID */
	...
	TWeakObjectPtr<UAITask_AeonixMoveTo> Task;
...
*/
UCLASS(config = Game)
class AEONIXNAVIGATION_API UBTTask_AeonixMoveTo : public UBTTask_BlackboardBase
{
	GENERATED_BODY()

	UBTTask_AeonixMoveTo(const FObjectInitializer& ObjectInitializer);
...
protected:
	...
	virtual UAITask_AeonixMoveTo* PrepareMoveTask(UBehaviorTreeComponent& OwnerComp, UAITask_AeonixMoveTo* ExistingTask, FAIMoveRequest& MoveRequest);
};
